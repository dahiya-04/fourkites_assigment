...
The framework is grounded in second-order optimization theory, utilizing the **Hessian Matrix** ($\mathbf{H}$) to quantify local curvature.

### Sharpness and Generalization

The **Sharpness** ($\mathcal{S}$) of a minimum $\mathbf{w}^*$ is defined by the maximum eigenvalue ($\lambda_{\max}$) of the Hessian matrix:

$$
\mathcal{S}(\mathbf{w}^*) \propto \lambda_{\max}(\mathbf{H}(\mathbf{w}^*))
$$

**Theoretical Result:** A smaller $\lambda_{\max}$ (flatter minimum) implies a smaller upper bound on the generalization error, suggesting that flatter minima are more robust to perturbations and generalize better.
